...
async function rebuildIndex(indexPath) {
  ...
  const exports = `export { ${componentFiles
...
export async function addFiles(inputPath, options = {}) {
  const cwd = process.cwd();
  if (!findPackageJson(cwd)) {
    console.log(
      chalk.red(
        "✖ Error: package.json not found. Make sure you are inside a Node.js project."
      )
    );
    process.exit(1);
  }
  const resolvedPath = resolveAliasPath(inputPath);
  const files = parseFileInput(
    resolvedPath,
    options.ext ? `.${options.ext}` : ".tsx"
  );
  for (const file of files) {
   ...
   if (!options.nocomp) {
      ...
      console.log(chalk.gray(`⏭ Skipped index.js update for ${folder} (due to --nocomp)`));
    }
  }
}